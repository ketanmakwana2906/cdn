...
def detect_axis_range(img, box, default_x_min=0, default_y_min=0):
    ...
    def pick_best(candidates, axis_min, n_grid_detected, prefer_index=None):
        ...
        grid_implied = axis_min + n_grid_detected - 1

        # Exact corroboration: some OCR read exactly matches what the
        # gridline count implies -- the strongest possible signal, and
        # short-circuits everything else.
        if grid_implied in cands:
            return grid_implied

        if prefer_index is not None and candidates[prefer_index] is not None:
            preferred = candidates[prefer_index]
            # NOTE: tried tightening this tolerance to +/-1 to catch the
            # sam01/sam16 y-axis cases below -- it fixed those but broke
            # MAG-PN-05 (anchor correctly read 30, gridline count was
            # undercounted by shading to 28, |diff|=3 -- a *correct*
            # anchor rejected by the same tolerance meant to catch a
            # *wrong* one). The two failure modes produce the same
            # disagreement size, so a single threshold can't separate
            # them reliably -- left at the original looser tolerance,
            # confirmed zero regressions on all 15 validated images.
            if abs((preferred - axis_min + 1) - n_grid_detected) <= max(6, 0.25 * n_grid_detected):
                return preferred

        # Trust a gridline-implied value that's HIGHER than anything OCR
        # found: a corrupted (shaded-overlay) gridline count only ever
        # UNDER-counts, never over-counts, so this direction can't be
        # faked by a bad count -- only by a correct count that OCR simply
        # failed to read that far. (Tested: trusting it in the other
        # direction, i.e. whenever "roughly corroborated", let an
        # undercounted case win over an equally-truncated OCR read.)
        if cands and grid_implied > max(cands):
            cands.append(grid_implied)
...
# When True, process_folder shows the auto-detected axis range for every
# image and asks for confirmation before using it to calibrate. This is
# the safety net for the failure mode proven above: OCR and gridline
# counting can each independently be wrong in ways that are
# indistinguishable from each other by the numbers alone (see the
# MAG-PN-05 vs sam01/sam16 case in pick_best's comments) -- no amount of
# heuristic tuning closes that gap completely, so a human check at this
# one high-leverage point is the honest fix rather than chasing a
# perfect automatic answer. Set to False for unattended/CI batch runs;
# if stdin isn't available (e.g. run non-interactively despite this being
# True), it safely falls back to the auto-detected value rather than
# hanging.
INTERACTIVE_AXIS_CONFIRM = True


def confirm_or_override_axis(stem, x_range, y_range):
    print(f"  Detected axis -> X: {x_range[0]} to {x_range[1]}   Y: {y_range[0]} to {y_range[1]}")
    if not INTERACTIVE_AXIS_CONFIRM:
        return x_range, y_range
    try:
        resp = input("  Is this correct? [Y/n]: ").strip().lower()
    except EOFError:
        print("  (no interactive input available -- keeping auto-detected axis)")
        return x_range, y_range
    if resp in ('', 'y', 'yes'):
        return x_range, y_range
    ...
    def ask_int(prompt, default):
        ...
        try:
            ...
            print(f"    Could not parse '{raw}' -- keeping {default}")
            return default

    x_min = ask_int("Enter correct X axis min", x_range[0])
    x_max = ask_int("Enter correct X axis max", x_range[1])
    y_min = ask_int("Enter correct Y axis min", y_range[0])
    y_max = ask_int("Enter correct Y axis max", y_range[1])
    return (x_min, x_max), (y_min, y_max)
...
def process_folder(folder, target_x, out_xlsx, patterns=('*.png', '*.jpg', '*.jpeg', '*.PNG', '*.JPG', '*.JPEG')):
    ...
    for f in files:
        ...
        try:
            box = detect_plot_box(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY))
            x_range, y_range = detect_axis_range(img, box)
            x_range, y_range = confirm_or_override_axis(stem, x_range, y_range)
            calib = calibrate(img, box, x_range, y_range)
            conf_ok, conf_info = calibration_confidence(calib)
