...
# ============================================================
# Canonical width every input image is normalized to before detection runs.
# The pipeline's internal thresholds (gridline deviation, regression
# tolerance, outlier windows) are defined in absolute pixel terms and were
# empirically tuned against charts in the ~1150-1550px-wide range. Testing
# showed accuracy silently degrades outside that band (uniform 2-3x
# upscaling and <0.5x downscaling both produced large, UNFLAGGED errors).
# Normalizing every input to this width first makes the pipeline correct
# regardless of the resolution the image arrives at -- validated to fully
# recover baseline accuracy for images that still contain enough native
# detail (see NORMALIZE tests). It cannot recover images that were already
# too low-resolution to read before normalization (real information loss,
# not a bug) -- MIN_SOURCE_WIDTH_PX below guards against silently
# processing those.
CANONICAL_WIDTH_PX = 1400
MIN_SOURCE_WIDTH_PX = 700  # below this, refuse rather than silently guess
NATIVE_BAND_PX = (1000, 1700)  # leave images in this band untouched (validated range)

# Plausibility band for this chart family's axis maxima, derived from all
# validated sample charts (observed: x_max always 30, y_max always 31-32).
# A generous +/-25% margin is used so legitimately different chart variants
# aren't falsely rejected -- this exists purely to catch OCR/gridline
# misreads that are internally self-consistent (and so pass the structural
# calibration_confidence check) but numerically implausible for this chart
# family, e.g. reading x_max=47 or x_max=10 instead of 30.
EXPECTED_X_MAX_RANGE = (22, 38)
EXPECTED_Y_MAX_RANGE = (23, 40)




def normalize_image(img):
    """Resize to CANONICAL_WIDTH_PX (preserving aspect ratio) ONLY when the
    source is outside the validated native band -- resizing an
    already-good image (even mildly) was measured to INTRODUCE error via
    interpolation blur, not remove it (native baseline mean error rose
    from 0.08 to 0.68 when every image was force-normalized). Leaving
    in-band images untouched and only correcting genuine outliers is the
    ...
    if NATIVE_BAND_PX[0] <= w <= NATIVE_BAND_PX[1]:
        return img, None  # already in the validated band -- leave untouched
...
def _robust_max_from_list(nums):
    ...
    corroborated by a nearby smaller value (part of the real ascending
    tick sequence) -- rejects isolated garbage reads (observed: colorized
    small tick text sometimes OCRs as a wildly larger stray number)."""
    if not nums:
        return None
    ...
    for v in reversed(vals[:-1]):
        if survivors[-1] - v <= 3:
            ...
            survivors = [v]
    return max(survivors) if len(survivors) >= 2 else (vals[-1] if len(vals) >= 3 else None)
...
def calibration_confidence(calib, tol_frac=0.03):
    ...
    x_plausible = EXPECTED_X_MAX_RANGE[0] <= x_max <= EXPECTED_X_MAX_RANGE[1]
    y_plausible = EXPECTED_Y_MAX_RANGE[0] <= y_max <= EXPECTED_Y_MAX_RANGE[1]
    ok = (w_err < tol_frac) and (h_err < tol_frac) and calib['n_x_gridlines_found'] >= 10 \
         and calib['n_y_gridlines_found'] >= 10 and x_plausible and y_plausible
    return ok, {'x_span_error_pct': round(w_err * 100, 2), 'y_span_error_pct': round(h_err * 100, 2),
                'x_range': calib['x_range'], 'y_range': calib['y_range'],
                'x_max_plausible': x_plausible, 'y_max_plausible': y_plausible}
...
def sample_at_x(cleaned_col_to_y, calib, x_query, half_window_px=1.5,
                 fit_half_window_px=6, min_fit_pts=5):
    """ENHANCEMENT (opt-in, additive): when the query column falls inside
    the curve's detected span, prefer a local tricube-weighted linear
    regression over a wider neighborhood (+/-6px) instead of a plain
    weighted average of the ~3 nearest columns. In high-curvature "knee"
    regions of these charts a plain neighbor-average is biased whenever
    point density/spacing is slightly asymmetric around the query column,
    because it isn't sensitive to local slope. A local line fit corrects
    that bias while reducing to essentially the same answer as the
    original method in flat regions -- so it is safe to apply everywhere.
    Falls back to the original nearest-neighbor/extrapolation logic
    verbatim if there aren't enough nearby points."""
    if not cleaned_col_to_y:
        return None
    c_center = calib['x_to_col'](x_query)
    cols_sorted_arr = np.array(sorted(cleaned_col_to_y))
    if cols_sorted_arr[0] <= c_center <= cols_sorted_arr[-1]:
        m = (cols_sorted_arr >= c_center - fit_half_window_px) & \
            (cols_sorted_arr <= c_center + fit_half_window_px)
        if m.sum() >= min_fit_pts:
            near_cols = cols_sorted_arr[m]
            near_ys = np.array([cleaned_col_to_y[c] for c in near_cols])
            d = np.abs(near_cols - c_center) / fit_half_window_px
            w = np.clip(1 - d ** 3, 0, 1) ** 3
            A = np.vstack([near_cols, np.ones(len(near_cols))]).T
            try:
                sol, *_ = np.linalg.lstsq(A * w[:, None], near_ys * w, rcond=None)
                slope, intercept = sol
                return float(slope * c_center + intercept)
            except np.linalg.LinAlgError:
                pass  # fall through to original method below
...
# ============================================================
# 6. GRAPH NAME (filename-based -- title OCR removed)
# ============================================================
# NOTE: v1/v2 also OCR'd the chart title purely to cross-check it against
# the filename in the audit log. It never fed calibration or the extracted
# values (process_image() -- the actual extraction path -- never called
# it). Measured zero accuracy impact from removing it; this only removes
# an extra OCR call per image and one more thing that could time out or
# misread on an unusual font, in exchange for the audit log naming a file
# by its filename instead of a cross-validated title.

def resolve_graph_name(img, filename_stem):
    return filename_stem, None, "filename (title OCR disabled)", False
...
def process_image(path, target_x, debug=False):
    ...
    if img is None:
        raise ValueError(f"Image too low-resolution to process reliably: {reason}")
